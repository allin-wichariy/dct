<?php
  ...
  if($id_grupo==181)  $descrip_grupo='1';
  if($id_grupo==182)  $descrip_grupo='1';
  if($id_grupo==183)  $descrip_grupo='1';
  if($id_grupo==191)  $descrip_grupo='2';
  if($id_grupo==192)  $descrip_grupo='2';  
  if($id_grupo==193)  $descrip_grupo='2';
 ...
 if($id_materia==6919 and $id_grupo>=90 and $id_grupo<= 100)
 	$name_aux=" (MODULO I)";
 elseif($id_materia==6920 and $id_grupo>=90 and $id_grupo<= 100)
 	$name_aux=" (MODULO II)";
 elseif($id_materia==6921 and $id_grupo>=90 and $id_grupo<= 100)
 	$name_aux=" (MODDULO III)";
 elseif($id_materia==6931 and $id_grupo>=100 )
 	$name_aux=" (TRIMESTRE I)";
 elseif($id_materia==6932 and $id_grupo>=100 )
 	$name_aux=" (TRIMESTRE II)";
 elseif($id_materia==6933 and $id_grupo>=100 )
 	$name_aux=" (TRIMESTRE III)";	

//$materia=$materia.$name_aux;	
if($id_grupo>=90 and $id_grupo<= 100){
	   $sigla=$sigla."  (INTENSIVO)";
	   }
  ...
  {
	$nota_aprobar = 80; //Nota minima de aprobacion de una materia dentro de la Universidad
  }     
  ...
  function encabezado($name_aux,$id_programa,$facultad,$programa,$nombresDocente,$sigla,$materia,$id_grupo,$cod_se,$id_periodo,$id_gestion,$descrip_grupo,$str_com,$cerrado){
	...
	if(trim($id_programa)=='EXQ')     
	{
		...
		$pdf->Cell(166,8,'PLANILLA DE NOTAS GESTION : '.$id_periodo."/".$id_gestion,0,1,'C');
	}
	else
	{
		if($id_periodo == 4 or $id_periodo == 5)
		{
			$_aux_id_periodo = (int)($id_periodo-3);
			$pdf->Cell(166,8,'PLANILLA DE NOTAS GESTION '.$_aux_id_periodo.'/'.$id_gestion." EXAMEN DE MESA",0,1,'C');
		}
		else
		{	
    ...
    // $pdf->Cell(166,4,'MATERIA: '.str_replace('�','N',$materia).'  ---  SIST. EVAL.: '.$cod_se,0,1,'L');
     $pdf->SetTextColor(150,0,0); $pdf->Cell(18,5,'MATERIA: ',0,0,'L');  $pdf->SetTextColor(10,10,150);   $pdf->Cell(148,5,$materia.' --- SIST. EVAL.: '.$cod_se,0,1,'L');     
     $pdf->SetTextColor(0,0,0);
