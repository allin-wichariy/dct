...
where v.id_viaje='$id_viajesito' and v.id_dct_asignaciones=a.id_dct_asignaciones and a.id_programa=p.id_programa and p.id_facultad=f.id_facultad and a.id_materia=m.id_materia and v.aprobado_dir='T'";
  ...
  $sql2="select cronograma,dia from infraestructura.cronograma where id='$id_viajesito' order by dia";
  $f->Ejecutar($sql2);
  if($f->filas!=0)
  {
      $arrayName = array();
  ...
  $sql3="select (u.paterno||' '||u.materno||' '||u.nombres)as nombres,u.nro_dip from infraestructura.alumnos_via a,alumnos au,uatf_datos u where id='$id_viajesito' and a.id_alumno=au.id_alumno and au.id_ra=u.id_ra and a.estado='A' order by nombres";
  ...
  if($c_f_pa>0)
  {
    $cqee="select nombre,tipo from infraestructura.pasajero_extra where id_viaje='$id_viajesito'";
    $f->Ejecutar($cqee);
  ...
  */
  class MiPdf extends FPDF
  {
    ...
    //var $periodo;
    //var $datos;
    public function __construct($id_viajeee)//$datos,$gestion,$periodo
    {
      parent::__construct('P','mm','Letter');
      $this->id_viajeee=$id_viajeee;
    ...
    public function Footer()
    {
      ...
      $this->text(105,260,'cod. Viaje '.$this->id_viajeee.' - Pag. '.$this->PageNo().' -');
    }
    ...
    public function crono($yyy,$algo_)
    {
      ...
      foreach ($algo_ as $key => $value) 
      {
          ...
          if($cuenta>7)
          {
            ...
            $yyy=$this->GetY();
            $this->cuadro($yyy);
            $this->MultiCell(196,4,utf8_decode('Cronograma Dia '.$value['dia'].' : '.$value['cronograma']),0,'J');
          }
          else
          {
            $this->SetY($yyy+16);
    ...
    public function estu($t,$nom_auxayudante,$nom_auxdocente)
    {
  ...
  $pdf= new MiPdf($id_viajesito);//$datos,$gestion,$periodo
  ...
  $pdf->cell(196,7,'NUMERO DE PARTICIPANTES :              Docentes   :   1,               Estudiantes   :   '.$cantidad_final_pas,1,1,'L');/*($pasaje-1),1,1,'L');*/
  ...
  $pdf->estu($arrayEstu,$nom_auxayudante,$nom_auxdocente);
