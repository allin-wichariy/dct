<?php
  ...
  $sql="select distinct v.id_dct_asignaciones,(m.materia ||' '||m.sigla)as materia,v.lugar_prac,v.fecha_ini,v.fecha_fin,v.fecha_r_ini,v.fecha_r_fin,v.pasajeros,v.id_viaje,a.id_grupo
  from infraestructura.viajes v,pln_materias m, dct_asignaciones a
  where a.id_dct_asignaciones=v.id_dct_asignaciones and a.id_docente='$id_docente' and a.id_gestion='$id_gestion' and a.id_periodo='$id_periodo' and m.id_materia=a.id_materia and v.aprobado_dir='T' and v.aprobado_dsa is null";
        //die($sql);
  //die($sql);
  $f->Ejecutar($sql);
  if ($f->filas == 0){
